...
TiDB supports changing the priority on a [per-session](reference/configuration/tidb-server/tidb-specific-variables.md#tidb-force-priority), [global](reference/configuration/tidb-server/configuration.md#force-priority) or individual statement basis. Priority has the following meaning:
