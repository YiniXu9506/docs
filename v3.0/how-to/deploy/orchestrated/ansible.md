...
category: how-to
aliases: ['/docs/v3.0/op-guide/ansible-deployment/']
---
...
> **Note:**
>
> If your data disks have been formatted to ext4 and have mounted the options, you can uninstall it by running the `# umount nvme0n1` command, follow the steps starting from editing the `/etc/fstab` file, and remount the filesystem with options.

Take the `nvme0n1` data disk as an example:

1. View the data disk.

    ```
    # fdisk -l
    Disk nvme0n1: 1000 GB
    ```

2. Create the partition table.

    ```
    # parted -s -a optimal nvme0n1 mklabel gpt -- mkpart primary ext4 1 -1
    ```
...
7. Check using the following command.
    ...
    ```
    # mount -t ext4
    nvme0n1 on /data1 type ext4 (rw,noatime,nodelalloc,data=ordered)
    ```
